...
if($version -ne "0.10.0"){ Fail "HARNESS_VERSION must be 0.10.0" }
...
foreach($needle in @("harness_version: `"0.10.0`"","claude-code","codex","gemini","generic-ai","kernel","preflight","denylists")){ if($registry -notmatch [regex]::Escape($needle)){ Fail "registry missing $needle" } }
