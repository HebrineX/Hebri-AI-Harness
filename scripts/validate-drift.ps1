...
if($version -ne "0.8.9"){ Fail "HARNESS_VERSION must be 0.8.9" }
$registry = Txt "orquestador/instruction-builder/instruction-registry.yaml"
foreach($needle in @("harness_version: `"0.8.9`"","claude-code","codex","gemini","generic-ai","kernel","preflight","denylists")){ if($registry -notmatch [regex]::Escape($needle)){ Fail "registry missing $needle" } }
