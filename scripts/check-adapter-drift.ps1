...
foreach($id in $expected){
  ...
  $yt = T $yaml
  [void](T $md)
  foreach($needle in @('harness_version: "0.8.9"','memory_reliability: untrusted_for_evidence','preflight_enforcement: required_before_effects','detractor_senior','first_message','reentry_light','reentry_full','debug_log_intake','compactation_recovery')){
